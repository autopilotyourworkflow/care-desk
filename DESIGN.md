...
| Desk | `/desk/` | Operate, full height | Three steps, one new thing at a time. First the queue alone: nothing opens on its own, and the empty area says "Your next message opens here" with one button that opens the first checked draft. Then the conversation: the patient's message with the reply straight under it (the checked draft, its sources folded to one line, and Send, Edit, Escalate; Pass to a person sits in the reply's More actions menu). With it, the details: the check trail folded to one line of seven ticks and the patient card (plan and order open, the rest under "More about"), under a "Details" header with a close button. They open with every message (his call, 2026-09-25); Hide details, beside the message title, closes them for that message only. On the tour the desk still opens one part at a time: the queue, then the message fading in, then at the check trail stop the details sliding in slowly. Queue rows carry one status chip (a safety row adds its reason and the hold). Keyboard first. The subtitle doubles as the first-visit hint ("Pick a message. Green means a checked draft is ready; anything clinical is already with a clinician."). No photos. |
| Clinician queue | `/clinician/` | Operate, full height | The same three steps: the list alone (the empty area offers the first clear urgent case), then the escalation with the order hold and the patient card open beside it (Hide details closes them for that item) (the number to call also sits by the call buttons). Escalations with the reason, then the message with the triggering words highlighted and one line naming what each word is ("What the AI thought" and the full trail are folded), the order hold, a clinician reply box (no AI; the activity log shows once there is something in it) and a crisis support card. The patient card keeps the place, local time, phone and any replies held back; the rest folds under "More about". No photos. |
...
- **Desk**: from 1024px a full-height grid: the queue (18.5rem) and the conversation (1fr), the message with the reply
  ...
  details 20rem; 22rem and 24rem at `2xl`) that opens with each message (0 wide once Hide details closes it): the track eases open over 300ms, the
  details slide in from the right edge at full width (never squeezed), and the conversation re-centres with it. This
  is the desk's one authored motion; on the tour it runs slowly (900ms, ease-in-out) at the check trail stop, the message
  column fades in (800ms after 300ms) at the reply and safety stops, and the tour card waits for each (TourStop.settleMs);
  none of it runs under reduced motion. Between 1024 and 1279px the
...
- **Clinician**: the same idea in two columns (queue 20rem to 24rem, detail 1fr). The escalation sits at 46rem; from
  `xl`, the item widens and a 340px column holds the hold and the patient, open by default.
